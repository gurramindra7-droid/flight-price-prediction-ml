...
# ---------------------------------------------------------------------------
# Logging (never logs request bodies, stack traces or filesystem paths)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
MODEL_PATH = (
    Path(__file__).resolve().parent.parent / "model" / "flight_price_model_compressed.joblib"
)
...
DURATION_MIN, DURATION_MAX = 0.1, 50.0
DAYS_LEFT_MIN, DAYS_LEFT_MAX = 1, 49
...
class PredictionRequest(BaseModel):
    airline: str = Field(..., description="One of the trained airline categories")
    source_city: str
    departure_time: str
    arrival_time: str
...
def validate_payload(payload: dict[str, Any]) -> tuple[dict[str, Any] | None, JSONResponse | None]:
    """Validate every input against the trained model's known categories.

    Returns (normalized_features, error_response). Exactly one is non-None.
    """
    required = (
        "airline",
        "source_city",
    ...
    for key in required:
        if key not in payload:
            return None, _field_error(key, "This field is required.")
    ...
    for key, allowed in (
        ...
        if not isinstance(value, str) or value not in allowed:
            return None, _field_error(key, f"Must be one of: {', '.join(allowed)}.")

    if payload["source_city"] == payload["destination_city"]:
        return None, _field_error(
            "destination_city", "Source and destination cities must be different."
        )

    # --- numerics ---------------------------------------------------------
    duration = payload["duration"]
    if isinstance(duration, bool) or not isinstance(duration, (int, float)):
        return None, _field_error("duration", "Duration must be a number (0.1 – 50 hours).")
    duration = float(duration)
    if duration != duration or duration in (float("inf"), float("-inf")):
        return None, _field_error("duration", "Duration must be a finite number.")
    if not (DURATION_MIN <= duration <= DURATION_MAX):
        return None, _field_error("duration", "Duration must be between 0.1 and 50 hours.")

    days_left = payload["days_left"]
    if isinstance(days_left, bool) or not isinstance(days_left, int):
        return None, _field_error("days_left", "Days left must be a whole number (1 – 49).")
    if not (DAYS_LEFT_MIN <= days_left <= DAYS_LEFT_MAX):
        return None, _field_error("days_left", "Days left must be between 1 and 49.")

    # --- feature engineering (identical to the original pipeline) ---------
    features = {
        ...
        "route": f"{payload['source_city']}_{payload['destination_city']}",
...
async def lifespan(_: FastAPI):
    # Warm the model at startup so the first request is not paying cold-start
    # deserialization twice (serverless runtimes reuse the process between
    # invocations whenever possible).
    try:
        get_model()
    except RuntimeError:
        pass  # surfaced as HTTP 503 on prediction requests
    yield
...
# Additional origins (e.g. a separately hosted frontend) can be added via the
...
async def health() -> dict[str, str]:
    if _model is not None:
        return {"status": "ok"}
    return {"status": "degraded" if _model_error else "ok"}


@app.post("/api/predict", response_model=PredictionResponse)
async def predict(request: Request) -> PredictionResponse | JSONResponse:
    try:
    ...
    if not isinstance(payload, dict):
        return _field_error("body", "Request body must be a JSON object.")
    ...
    try:
        ...
        if price != price:  # NaN guard
            raise ValueError("model produced a non-finite price")
        ...
        return JSONResponse(
            ...
            content={
                "error": {
                    ...
                    "message": "Prediction could not be completed. Please try again.",
                }
            },
        )
